...
class ConverterApp(MDApp):
    def flip(self):
        if self.state == 0:
            self.state = 1
            self.toolbar.title = "Enter a Decimal Number"
            self.input.text = ""
        else:
            self.state = 0
            self.toolbar.title = "Enter a Binary Number"
            self.input.text = ""
        self.converted.text = ""
        self.label.text = ""
    ...
    def build(self):
        ...
        screen = MDScreen()
        self.toolbar = MDToolbar(title="Enter a Binary Number")
        self.toolbar.post_hint = {"top": 1}
        self.toolbar.right_action_items = [["rotate-3d-variant", lambda x: self.flip()]]
        screen.add_widget(self.toolbar)
        screen.add_widget(Image(source="logo.jpg", pos_hint = {"center_x": 0.5, "center_y": 0.9}, width=200, allow_stretch=True))
        self.input = MDTextField(text="", halign="center", size_hint=(0.8, 1), pos_hint={"center_x": 0.5, "center_y": 0.6}, font_size=22)
        ...
        screen.add_widget(MDFillRoundFlatButton(text="CONVERT", font_size=17, pos_hint={"center_x": 0.5, "center_y": 0.7}, on_press=self.convert))
        return screen
